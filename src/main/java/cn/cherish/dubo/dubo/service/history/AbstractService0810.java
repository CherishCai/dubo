package cn.cherish.dubo.dubo.service.history;
 ...
 */
@Slf4j
public abstract class AbstractService0810 {
    ...
    protected Term newTerm(History.RowsBean rowsBean) {
...
//                failureList.add(task);
     ...
     * 24_680
     */
    protected void rule24_680(final List<Term> terms) {
        String mmm = "顶24_底680";
        List<Set<Integer>> rules = Rule2Utils.rules24_680();
     ...
     * 单双 ❌ 五次
     */
    protected void evenOddTick(final List<Term> terms, final int skip, final int evenOddTickNum) {
        List<Term> list = terms;
        if (CollectionUtils.isEmpty(list)) {
            return;
        }
     ...
     * 大小 ❌ 五次
     */
    protected void bigSmallTick(final List<Term> terms, final int skip, final int evenOddTickNum) {
        List<Term> list = terms;
        if (CollectionUtils.isEmpty(list)) {
            return;
        }
